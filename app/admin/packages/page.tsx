...
import { Search, Plus, Users, Wallet, Pencil, Trash2 } from "lucide-react"
  ...
  return (
    <main className="max-w-7xl mx-auto px-6 py-12 relative z-20">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => setShowCreate(true)} className="bg-blue-200/80 hover:bg-blue-300 text-white font-medium px-5 py-3 rounded-lg flex items-center gap-2">
          + Create New Package
        </button>
      </div>
        ...
        {loading ? (
          ...
          packages.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).map((pkg) => (
            <div key={pkg._id} className="bg-white/50 rounded-2xl p-6 shadow-xl border-2 border-gray-100">
              ...
              {/* Tier Badge */}
              <div className="mb-4">
                <span className={`${getTierColor(pkg.tier)} px-3 py-1 rounded-full text-sm font-medium inline-block`}>{pkg.tier}</span>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {pkg.items.map((item, idx) => (
                    <span key={idx} className="bg-red-100 text-red-700 px-3 py-1 rounded-full text-sm font-medium">{item}</span>
                  ))}
                </div>
              </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <form onSubmit={handleCreate} className="bg-white rounded-lg p-6 w-[640px]">
